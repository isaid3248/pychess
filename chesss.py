...
os.chdir("..") # moves one file backwards.
...
class ChessGUI:
    ...
    # Function that updates two squares at the same time.
    ...
    def initialize_board(self, board):
        ...
        self.layout[0].append(
            pg.Column(self.columnA, scrollable=False, element_justification="right", size=(resolution, resolution)))
    ...
    def run(self):
        ...
        # window variable is defined here and not in __init__.
        ...
        # board is defined here and not in __init__.
        ...
        while True:
            ...
            # Second click checks if the move is possible then plays it, if not then cancels.
            elif click == 1 and previousEvent != "Undo-Move":
                ...
                if (click == 1):
                    self.window[previousEvent].update(button_color=self.self.colors[(previousEvent[0] + previousEvent[1]) % 2])

            #for i in self.board: print(i)

        self.window.close()  # don't forget thisss